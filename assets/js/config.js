...
window.SS_CONFIG = {
  ...
  /* ── درگاه پرداخت آنلاین زیبال ──
     merchant = کد ۳۲ رقمی مرچنت شما از پنل زیبال (panel.zibal.ir).
     تا وقتی مقدار «zibal» باشد، درگاه در حالت تست (سندباکس) است:
       درگاه تست زیبال باز می‌شود و پرداخت واقعی انجام نمی‌شود.
     بعد از تأیید درگاه در پنل، فقط همین مقدار را با مرچنت‌کد واقعی عوض کنید
     و بقیه سیستم (درخواست، بازگشت، وریفای، نوتیف تلگرام) خودکار کار می‌کند. */
  zibal: {
    enabled: true,
    merchant: 'zibal'
  },
